...
export const createChat = (chatData) => async (dispatch) => {
  try {
    ...
    dispatch({ type: CREATE_CHAT, payload: null });
  }
};
...
export const createGroupChat = (chatData) => async (dispatch) => {
  try {
    ...
    console.log("catch error ", error);
    dispatch({ type: CREATE_GROUP, payload: null });
  }
};
...
export const getUsersChat = (chatData) => async (dispatch) => {
  try {
    ...
    // Guard: only dispatch if the response is actually an array
    dispatch({ type: GET_USERS_CHAT, payload: Array.isArray(data) ? data : [] });
  } catch (error) {
    console.log("catch error ", error);
